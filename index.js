    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; padding: 40px; background: #F7F3EE;">
          ...
          <p style="margin-top: 24px; font-size: 12px; color: #A89880;">Saved to Airtable.</p>
        </div>
      `
    });
    res.json({ ok: true });
...
app.post("/notify-private", async (req, res) => {
  try {
    const { name, email, phone, ecName, ecPhone, physical, experience, intentions, other } = req.body;
    await transporter.sendMail({
      from: "tory@indianpondbarbados.com",
      to: "tory@indianpondbarbados.com",
      subject: `New private yoga registration: ${name}`,
      html: `
        <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; padding: 40px; background: #F7F3EE;">
          <h2 style="font-weight: 300; color: #2C2416; border-bottom: 1px solid #E8DDD0; padding-bottom: 16px;">New Private Yoga Client</h2>
          <table style="width: 100%; border-collapse: collapse; font-size: 14px; color: #2C2416;">
            <tr><td style="padding: 10px 0; color: #A89880; width: 200px;">Name</td><td style="padding: 10px 0;"><strong>${name}</strong></td></tr>
            <tr><td style="padding: 10px 0; color: #A89880;">Email</td><td style="padding: 10px 0;">${email}</td></tr>
            <tr><td style="padding: 10px 0; color: #A89880;">Phone</td><td style="padding: 10px 0;">${phone}</td></tr>
            <tr><td style="padding: 10px 0; color: #A89880;">Emergency Contact</td><td style="padding: 10px 0;">${ecName} — ${ecPhone}</td></tr>
            <tr><td style="padding: 10px 0; color: #A89880; vertical-align: top;">Physical Considerations</td><td style="padding: 10px 0;">${physical || "None stated"}</td></tr>
            <tr><td style="padding: 10px 0; color: #A89880; vertical-align: top;">Previous Experience</td><td style="padding: 10px 0;">${experience || "None stated"}</td></tr>
            <tr><td style="padding: 10px 0; color: #A89880; vertical-align: top;">Intentions</td><td style="padding: 10px 0;">${intentions || "None stated"}</td></tr>
            <tr><td style="padding: 10px 0; color: #A89880; vertical-align: top;">Other Notes</td><td style="padding: 10px 0;">${other || "None stated"}</td></tr>
            <tr><td style="padding: 10px 0; color: #A89880;">Waiver Signed</td><td style="padding: 10px 0;">Yes</td></tr>
          </table>
          <p style="margin-top: 24px; font-size: 12px; color: #A89880;">Saved to Airtable Private Clients table.</p>
        </div>
      `
    });
    res.json({ ok: true });
  } catch (e) {
    console.error("Private notify error:", e.message);
    res.status(500).json({ error: e.message });
  }
});
